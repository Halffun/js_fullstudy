...
// 创建 CanvasBarrage 类
class CanvasBarrage {
  constructor(canvas, video, opts = {}) { // constructor 是一种用于创建和初始化 class 对象的方法
    // opts = {} 表示如果没有传值，默认为{}，防止报错，ES6的语法
    ...
    // 合并对象，再挂载到this上
    Object.assign(this, defOpts, opts) // opts 覆盖 defOpts, opts和defOpts合并后给this, 也就是这个作用域
  ...
  renderBarrage(){
    ...
    this.barrages.forEach(barrage => {
      ...
      // 并且只有在视频播放的时间大于等于当前弹幕的展示时间时，才处理
      if (!barrage.flag && time >= barrage.time){
        // 判断当前这条弹幕是否被初始化过了
        // 如果isInit是false，那么需要对当前的弹幕进行初始化操作
        if(!barrage.isInit){
          barrage.init()
          barrage.isInit = true
        }
...
function send(){
  ...
  let time = video.currentTime // 当前视频播放的时间进度
